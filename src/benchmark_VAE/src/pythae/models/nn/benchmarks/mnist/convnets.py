...
class Encoder_Conv_VAE_MNIST(BaseEncoder):
    ...
    def __init__(self, args: BaseAEConfig):
        ...
        layers.append(
            nn.Sequential(
                nn.Conv2d(self.n_channels, 128, 4, 2, padding=1), nn.BatchNorm2d(128), nn.ReLU(),
            )
        ) #56
        ...
        layers.append(
            nn.Sequential(
                nn.Conv2d(1024, 1024, 3, 2, padding=1), nn.BatchNorm2d(1024), nn.ReLU()
            )
        ) #4

        layers.append(
            nn.Sequential(
                nn.Conv2d(1024, 1024, 4, 2, padding=1), nn.BatchNorm2d(1024), nn.ReLU()
            )
        ) #4
        ...
        self.embedding = nn.Linear(4096, args.latent_dim)
        self.log_var = nn.Linear(4096, args.latent_dim)
...
class Decoder_Conv_AE_MNIST(BaseDecoder):
    ...
    def __init__(self, args: dict):
        ...
        layers.append(nn.Linear(args.latent_dim, 1024 * 2 * 2))
        ...
        layers.append(
            nn.Sequential(
                nn.ConvTranspose2d(1024, 1024, 3, 2, padding=1),
                nn.BatchNorm2d(1024),
                nn.ReLU(),
            )
        ) #7
